...
80000234:	04700293          	li	t0,71
    beq a0, t0, .OP_G
80000238:	12550063          	beq	a0,t0,80000358 <.OP_G>
    li t0, 'T'
...
80000248:	72c000ef          	jal	ra,80000974 <WRITE_SERIAL>
    j .DONE
8000024c:	2e80006f          	j	80000534 <.DONE>
...
8000028c:	00c10113          	addi	sp,sp,12
    ...
    j .DONE
80000290:	2a40006f          	j	80000534 <.DONE>
...
800002c8:	00810113          	addi	sp,sp,8
    j .DONE
800002cc:	2680006f          	j	80000534 <.DONE>
...
80000308:	00810113          	addi	sp,sp,8
    j .DONE
8000030c:	2280006f          	j	80000534 <.DONE>
...
80000344:	0000100f          	fence.i
    ...
    // 恢复 s1, s2
    LOAD s1, 0(sp)
80000348:	00012483          	lw	s1,0(sp)
    LOAD s2, XLEN(sp)
8000034c:	00412903          	lw	s2,4(sp)
    addi sp, sp, 2*XLEN
80000350:	00810113          	addi	sp,sp,8
    j .DONE
80000354:	1e00006f          	j	80000534 <.DONE>
...
.OP_G:
    ...
    // 获取 addr
    jal READ_SERIAL_XLEN
80000358:	75c000ef          	jal	ra,80000ab4 <READ_SERIAL_XLEN>
    // 保存到 s10
    mv s10, a0
8000035c:	00050d13          	mv	s10,a0

    // 写开始计时信号
    // 告诉终端用户程序开始运行
    li a0, SIG_TIMERSET
80000360:	00600513          	li	a0,6
    jal WRITE_SERIAL
80000364:	610000ef          	jal	ra,80000974 <WRITE_SERIAL>

#ifdef ENABLE_INT
    // 用户程序入口写入EPC
    csrw mepc, s10
80000368:	341d1073          	csrw	mepc,s10

    // 设置 MPP=0，对应 U-mode
    li a0, MSTATUS_MPP_MASK
8000036c:	00002537          	lui	a0,0x2
80000370:	80050513          	addi	a0,a0,-2048 # 1800 <INITLOCATE-0x7fffe800>
    csrc mstatus, a0
80000374:	30053073          	csrc	mstatus,a0
    li t3, 10000000
...
#else
    li t0, CLINT_MTIME
80000378:	0200c2b7          	lui	t0,0x200c
8000037c:	ff828293          	addi	t0,t0,-8 # 200bff8 <INITLOCATE-0x7dff4008>
    lw t1, 0(t0)        // 读取 mtime 低 32 位
80000380:	0002a303          	lw	t1,0(t0)
    lw t2, 4(t0)        // 读取 mtime 高 32 位
80000384:	0042a383          	lw	t2,4(t0)
    li t3, 100000
80000388:	00018e37          	lui	t3,0x18
8000038c:	6a0e0e13          	addi	t3,t3,1696 # 186a0 <INITLOCATE-0x7ffe7960>
    add t3, t1, t3      // 低 32 位 + 10000000
80000390:	01c30e33          	add	t3,t1,t3
    sltu t1, t3, t1     // 生成进位，若进位 t1 = 1
80000394:	006e3333          	sltu	t1,t3,t1
    add t2, t2, t1      // 高 32 位进位
80000398:	006383b3          	add	t2,t2,t1
    li t0, CLINT_MTIMECMP
8000039c:	020042b7          	lui	t0,0x2004
    sw t2, 4(t0)        // 写入 mtimecmp 高 32 位
800003a0:	0072a223          	sw	t2,4(t0) # 2004004 <INITLOCATE-0x7dffbffc>
    sw t3, 0(t0)        // 写入 mtimecmp 低 32 位
800003a4:	01c2a023          	sw	t3,0(t0)
#endif
    ...
    // 定位用户空间寄存器备份地址
    la ra, uregs
800003a8:	007f0097          	auipc	ra,0x7f0
800003ac:	c5808093          	addi	ra,ra,-936 # 807f0000 <_sbss>
    // 保存栈指针
    STORE sp, TF_ksp(ra)
800003b0:	0820a023          	sw	sp,128(ra)

    // x1 就是 ra
    // LOAD x1,  TF_ra(ra)
    LOAD sp, TF_sp(ra)
800003b4:	0040a103          	lw	sp,4(ra)
    LOAD gp, TF_gp(ra)
800003b8:	0080a183          	lw	gp,8(ra)
    LOAD tp, TF_tp(ra)
800003bc:	00c0a203          	lw	tp,12(ra)
    LOAD t0, TF_t0(ra)
800003c0:	0100a283          	lw	t0,16(ra)
    LOAD t1, TF_t1(ra)
800003c4:	0140a303          	lw	t1,20(ra)
    LOAD t2, TF_t2(ra)
800003c8:	0180a383          	lw	t2,24(ra)
    LOAD s0, TF_s0(ra)
800003cc:	01c0a403          	lw	s0,28(ra)
    LOAD s1, TF_s1(ra)
800003d0:	0200a483          	lw	s1,32(ra)
    LOAD a0, TF_a0(ra)
800003d4:	0240a503          	lw	a0,36(ra)
    LOAD a1, TF_a1(ra)
800003d8:	0280a583          	lw	a1,40(ra)
    LOAD a2, TF_a2(ra)
800003dc:	02c0a603          	lw	a2,44(ra)
    LOAD a3, TF_a3(ra)
800003e0:	0300a683          	lw	a3,48(ra)
    LOAD a4, TF_a4(ra)
800003e4:	0340a703          	lw	a4,52(ra)
    LOAD a5, TF_a5(ra)
800003e8:	0380a783          	lw	a5,56(ra)
    LOAD a6, TF_a6(ra)
800003ec:	03c0a803          	lw	a6,60(ra)
    LOAD a7, TF_a7(ra)
800003f0:	0400a883          	lw	a7,64(ra)
    LOAD s2, TF_s2(ra)
800003f4:	0440a903          	lw	s2,68(ra)
    LOAD s3, TF_s3(ra)
800003f8:	0480a983          	lw	s3,72(ra)
    LOAD s4, TF_s4(ra)
800003fc:	04c0aa03          	lw	s4,76(ra)
    LOAD s5, TF_s5(ra)
80000400:	0500aa83          	lw	s5,80(ra)
    LOAD s6, TF_s6(ra)
80000404:	0540ab03          	lw	s6,84(ra)
    LOAD s7, TF_s7(ra)
80000408:	0580ab83          	lw	s7,88(ra)
    LOAD s8, TF_s8(ra)
8000040c:	05c0ac03          	lw	s8,92(ra)
    LOAD s9, TF_s9(ra)
80000410:	0600ac83          	lw	s9,96(ra)
    // s10 用来保存用户程序地址
    // LOAD s10, TF_s10(ra)
    LOAD s11, TF_s11(ra)
80000414:	0680ad83          	lw	s11,104(ra)
    LOAD t3, TF_t3(ra)
80000418:	06c0ae03          	lw	t3,108(ra)
    LOAD t4, TF_t4(ra)
8000041c:	0700ae83          	lw	t4,112(ra)
    LOAD t5, TF_t5(ra)
80000420:	0740af03          	lw	t5,116(ra)
    LOAD t6, TF_t6(ra)
80000424:	0780af83          	lw	t6,120(ra)
...
80000430:	00c08093          	addi	ra,ra,12 # 80000438 <.USERRET_USER>
    // 进入用户程序
    mret
80000434:	30200073          	mret

80000438 <.USERRET_USER>:
...
80000440:	0ff0000f          	fence
    ...
    // 告诉终端用户程序结束运行
    li a0, SIG_TIMEOUT
80000444:	08100513          	li	a0,129
    jal WRITE_SERIAL
80000448:	52c000ef          	jal	ra,80000974 <WRITE_SERIAL>
    j 0f
8000044c:	0100006f          	j	8000045c <USERRET_MACHINE+0xc>

80000450 <USERRET_MACHINE>:
...
80000450:	0ff0000f          	fence
    ...
    // 告诉终端用户程序结束运行
    li a0, SIG_TIMETOKEN
80000454:	00700513          	li	a0,7
    jal WRITE_SERIAL
80000458:	51c000ef          	jal	ra,80000974 <WRITE_SERIAL>

    // 复制寄存器数据
0:
    la s1, uregs
8000045c:	007f0497          	auipc	s1,0x7f0
80000460:	ba448493          	addi	s1,s1,-1116 # 807f0000 <_sbss>
    li s2, TF_SIZE
80000464:	08000913          	li	s2,128
.LC4:
    lw a0, 0(sp)
80000468:	00012503          	lw	a0,0(sp)
    sw a0, 0(s1)
8000046c:	00a4a023          	sw	a0,0(s1)
    addi s2, s2, -4
80000470:	ffc90913          	addi	s2,s2,-4
    addi s1, s1, 0x4
80000474:	00448493          	addi	s1,s1,4
    addi sp, sp, 0x4
80000478:	00410113          	addi	sp,sp,4
    bne s2, zero, .LC4
8000047c:	fe0916e3          	bnez	s2,80000468 <USERRET_MACHINE+0x18>

    // 重新获得当前监控程序栈顶指针
    la s1, uregs
80000480:	007f0497          	auipc	s1,0x7f0
80000484:	b8048493          	addi	s1,s1,-1152 # 807f0000 <_sbss>
    LOAD sp, TF_ksp(s1)
80000488:	0804a103          	lw	sp,128(s1)

    j .DONE
8000048c:	0a80006f          	j	80000534 <.DONE>
...
.USERRET2:
    // 定位用户空间寄存器备份地址
    la ra, uregs
80000490:	007f0097          	auipc	ra,0x7f0
80000494:	b7008093          	addi	ra,ra,-1168 # 807f0000 <_sbss>

    // 不能先恢复 ra
    //STORE ra, TF_ra(ra)
    STORE sp, TF_sp(ra)
80000498:	0020a223          	sw	sp,4(ra)
    STORE gp, TF_gp(ra)
8000049c:	0030a423          	sw	gp,8(ra)
    STORE tp, TF_tp(ra)
800004a0:	0040a623          	sw	tp,12(ra)
    STORE t0, TF_t0(ra)
800004a4:	0050a823          	sw	t0,16(ra)
    STORE t1, TF_t1(ra)
800004a8:	0060aa23          	sw	t1,20(ra)
    STORE t2, TF_t2(ra)
800004ac:	0070ac23          	sw	t2,24(ra)
    STORE s0, TF_s0(ra)
800004b0:	0080ae23          	sw	s0,28(ra)
    STORE s1, TF_s1(ra)
800004b4:	0290a023          	sw	s1,32(ra)
    STORE a0, TF_a0(ra)
800004b8:	02a0a223          	sw	a0,36(ra)
    STORE a1, TF_a1(ra)
800004bc:	02b0a423          	sw	a1,40(ra)
    STORE a2, TF_a2(ra)
800004c0:	02c0a623          	sw	a2,44(ra)
    STORE a3, TF_a3(ra)
800004c4:	02d0a823          	sw	a3,48(ra)
    STORE a4, TF_a4(ra)
800004c8:	02e0aa23          	sw	a4,52(ra)
    STORE a5, TF_a5(ra)
800004cc:	02f0ac23          	sw	a5,56(ra)
    STORE a6, TF_a6(ra)
800004d0:	0300ae23          	sw	a6,60(ra)
    STORE a7, TF_a7(ra)
800004d4:	0510a023          	sw	a7,64(ra)
    STORE s2, TF_s2(ra)
800004d8:	0520a223          	sw	s2,68(ra)
    STORE s3, TF_s3(ra)
800004dc:	0530a423          	sw	s3,72(ra)
    STORE s4, TF_s4(ra)
800004e0:	0540a623          	sw	s4,76(ra)
    STORE s5, TF_s5(ra)
800004e4:	0550a823          	sw	s5,80(ra)
    STORE s6, TF_s6(ra)
800004e8:	0560aa23          	sw	s6,84(ra)
    STORE s7, TF_s7(ra)
800004ec:	0570ac23          	sw	s7,88(ra)
    STORE s8, TF_s8(ra)
800004f0:	0580ae23          	sw	s8,92(ra)
    STORE s9, TF_s9(ra)
800004f4:	0790a023          	sw	s9,96(ra)
    STORE s10, TF_s10(ra)
800004f8:	07a0a223          	sw	s10,100(ra)
    STORE s11, TF_s11(ra)
800004fc:	07b0a423          	sw	s11,104(ra)
    STORE t3, TF_t3(ra)
80000500:	07c0a623          	sw	t3,108(ra)
    STORE t4, TF_t4(ra)
80000504:	07d0a823          	sw	t4,112(ra)
    STORE t5, TF_t5(ra)
80000508:	07e0aa23          	sw	t5,116(ra)
    STORE t6, TF_t6(ra)
8000050c:	07f0ac23          	sw	t6,120(ra)

    // 重新获得当前监控程序栈顶指针
    LOAD sp, TF_ksp(ra)
80000510:	0800a103          	lw	sp,128(ra)
    mv a0, ra
80000514:	00008513          	mv	a0,ra
    la ra, .USERRET2
80000518:	00000097          	auipc	ra,0x0
8000051c:	f7808093          	addi	ra,ra,-136 # 80000490 <.USERRET2>
    STORE ra, TF_ra(a0)
80000520:	00152023          	sw	ra,0(a0)

    // 发送停止计时信号
    li a0, SIG_TIMETOKEN
80000524:	00700513          	li	a0,7
    // 告诉终端用户程序结束运行
    jal WRITE_SERIAL
80000528:	44c000ef          	jal	ra,80000974 <WRITE_SERIAL>
...
8000052c:	0ff0000f          	fence
    ...
    j .DONE
80000530:	0040006f          	j	80000534 <.DONE>
...
.DONE:
    // 交互循环
    j SHELL
80000534:	ce5ff06f          	j	80000218 <SHELL>
	...
...
.HANDLE_BREAK:
    j USERRET_MACHINE
800006e4:	d6dff06f          	j	80000450 <USERRET_MACHINE>
...
800006fc:	00029463          	bnez	t0,80000704 <CONTEXT_SWITCH>
    ...
    // 处理用户程序超时
    j USERRET_TIMEOUT
80000700:	d41ff06f          	j	80000440 <USERRET_TIMEOUT>
...
80001008 <UTEST_1PTB>:
     ...
     *  执行这段程序需至少 384M 指令，384M/time 可算得频率。
     */
UTEST_1PTB:
